class Marketing::Email::MessagesController < Marketing::MarketingBaseController
  ...
  def initialize
    @klass_associate_template = :marketing_email_templates
  end
  
  def index
    @messages = current_user.email_messages.paginate(page: params[:email_messages_page], per_page: 10,
                                              total_entries: current_user.email_messages.length)
  end
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      if @message.try(:sent).blank? && @message.try(:destroy)
        @messages = current_user.email_messages.paginate(page: params[:email_messages_page], per_page: 10,
                                              total_entries: current_user.email_messages.length)
        format.js{}
        format.html{
        ...
        format.js{
          render text: ";$('#flash-error').html('Something is wrong with your smtp setting.');", status: 401
        }
        format.html{
  ...
  def send_message
    ...
    respond_to do |format|
      unless receiver_emails(@message).blank?
        ...
        format.js{
          render text: ";$('#flash-error').html('Not sent because any of your receiver does not have email address.');", status: 401
        }
        format.html{
