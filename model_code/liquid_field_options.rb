module LiquidFieldOptions
  ...
  def template_body_dynamic_options
    ...
    %w[connection agent case_manager profile].each do |has_contact_info|
     ...
     %w[name email phone signature].each do |field|
        items << {:label => "agent_of_record_#{field}", :value => "agent_of_record_#{field}"} 
      end
  ...
  def template_body_liquid_values
    ...
    %w[connection agent case_manager profile].each do |has_contact_info|
      %w[email phone zip fax address city].each do |field|
        items["#{has_contact_info}_#{field}"] = "#{has_contact_info}.#{field}" 
      end
    end

    %w[connection agent case_manager profile].each do |has_contact_info|
      items["#{has_contact_info}_state"] = "#{has_contact_info}.state.name" 
    end
    
    %w[name email phone signature].each do |field|
      items["agent_of_record_#{field}"] = "agent_of_record.#{field}" 
    end
    ...
    %w[quoted submitted approved].each do |app_status|
      %w[carrier_name planned_modal_premium premium_mode].each do |field|
